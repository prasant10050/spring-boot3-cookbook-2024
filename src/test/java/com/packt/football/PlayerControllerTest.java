...
public class PlayerControllerTest {
    ...
    @Test
    public void testReadPlayer_doesnt_exists() throws Exception {
        String id = "1884823";
    ...
    @Test
    public void testReadPlayer_does_exists() throws Exception {
        // ARRANGE
        ...
        ObjectMapper mapper = new ObjectMapper();
        mapper.registerModule(new JavaTimeModule());
        MvcResult result = mvc.perform(MockMvcRequestBuilders.get("/players/" + id).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andReturn();
        String json = result.getResponse().getContentAsString();
    ...
    public void testDeletePlayer() throws Exception {
        String id = "1884823";
        mvc.perform(MockMvcRequestBuilders.delete("/players/" + id).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
    }
    ...
    public void testCreatePlayer_already_exists() throws Exception {
        ...
        Player player = new Player("1884823", 5, "Ivana ANDRES", "Defender", LocalDate.of(1994, 07, 13));
        given(footballService.addPlayer(player))
                .willThrow(new AlreadyExistsException("The player already exists"));
        ObjectMapper mapper = new ObjectMapper();
        mapper.registerModule(new JavaTimeModule());
        mvc.perform(MockMvcRequestBuilders.post("/players")
                        .content(mapper.writeValueAsString(player))
    ...
    public void testUpdatePlayer_doesnt_exists() throws Exception {
        // ARRANGE
        Player player = new Player("1884823", 5, "Ivana ANDRES", "Defender", LocalDate.of(1994, 07, 13));
        given(footballService.updatePlayer(player))
